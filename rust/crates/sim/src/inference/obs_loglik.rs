...
/// other helpers. The NaN arm is written out rather than folded into `<=`: a
/// NaN comparison is false, so a bare `sigma <= 0.0` let a NaN scale through
...
/// as every other domain violation in this module does. The NaN arm is written
/// out rather than folded into `<=`: a NaN comparison is false, so a bare
...
pub fn beta_logpdf_grad(x: f64, mean: f64, concentration: f64) -> (f64, f64) {
    let a = mean * concentration;
    let b = (1.0 - mean) * concentration;
    // Every domain arm carries its own NaN test: a NaN compares false against
    // `<=` and `>=` alike, so the bare comparisons would let a NaN observation
    // or a NaN shape walk into `ln`/`digamma` and return a NaN gradient. `a`
    // and `b` are NaN whenever `mean` or `concentration` is, so testing them
    // covers the arguments too.
    if x.is_nan()
        || x <= 0.0
        || x >= 1.0
        || a.is_nan()
