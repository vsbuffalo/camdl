...
enum QProgram {
    Reduced { source: ResolvedExpr, reduce: Option<RReduce> },
...
impl QuantityEvaluator {
        ...
        for q in quantities {
            let prog = match &q.body {
                QuantityBody::Reduced { source, reduce } => {
                    let QuantitySource::State(expr) = source;
                    let source = resolve(expr)?;
    ...
    /// vector, `traj` the finished trajectory. Pure; results are in the same order
    /// as the `quantities` list (so a `Derived` can read prior scalars).
    pub fn eval_draw(&self, params: &[f64], traj: &Trajectory, compiled: &CompiledModel) -> Vec<QuantityResult> {
        ...
        for prog in &self.programs {
            let r = match prog {
                QProgram::Reduced { source, reduce } => {
                    let series = eval_series(source, traj, compiled, params);
                    match reduce {
                        None => QuantityResult::Series(series),
                        Some(red) => {
                            QuantityResult::Scalar(fold_reduce(red, &series, &times, traj, compiled, params))
                        }
                    }
                }
...
/// Uses the snapshot's own state directly (the post-hoc truth); no integrator
...
fn fold_reduce(
    ...
    let thresh = |e: &ResolvedExpr| eval_series(e, traj, compiled, params);
    match r {
