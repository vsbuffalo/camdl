    ...
    while pc < n {
        match ops.get_unchecked(pc) {
            ...
            Op::JumpIfFalse(t) => {
                ...
                sp -= 1;
                // Take the else branch whenever the predicate is not strictly
                // positive, NaN included — matching `eval_resolved`. The NaN
                // arm is explicit because `pred <= 0.0` alone is false for NaN.
