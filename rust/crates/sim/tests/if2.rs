...
fn generate_data(compiled: &CompiledModel, params: &[f64]) -> (Vec<f64>, Vec<f64>) {
    ...
    while t < 77.0 {
        for _ in 0..7 {
            ...
            step_one(compiled, &mut state.counts, &mut state.flow_accumulators, &mut real, params, t, 1.0, None, sim::rng::BinomialAlgorithm::default(), &mut rng, &mut scratch).unwrap();
            t += 1.0;
        }
