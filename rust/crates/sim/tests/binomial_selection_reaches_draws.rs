...
//!
//! The `binomial` knob travels **as a value** from the typed stage field down
//! to `step_one` (proposal `2026-08-24-faster-binomial-sampler.md` §1),
//! because the draws happen on rayon workers inside nested `par_iter`s where
//! a thread-local set by the configuring thread is invisible. These tests pin
//! the threading itself: if any link in the chain drops the argument and
//! falls back to a default, the BTRS run below becomes byte-identical to the
...
fn reference_with(algo: BinomialAlgorithm) -> PGASTrajectory {
    ...
    simulate_reference(&compiled, &params, N_SUBSTEPS as f64 * DT, DT, algo, &mut rng)
        .expect("reference")
}
