...
//! cannot be resolved, a scenario window this verb cannot honour. One
//! occurrence is a defect, so the run names the site that produced it and
//! exits 1.
//!
...
//! computes two things that fail independently — the data-conditioned
//! one-step-ahead band, which never reaches the forecast horizon, and the
//! free-forward tail — and refusing to write the first because the second
//! refused loses work that was finished and correct. So: compute what is
//! computable, write it, name what failed in `report.json`, and pass nothing
...
///
/// Serializes externally-tagged, so a unit variant is a bare string:
/// `"at": "free_forward"`.
...
mod tests {
    ...
    #[test]
    fn a_failure_names_its_site_and_its_reason_in_the_json() {
        let dir = crate::test_support::unique_temp_dir("predict_report_failed");
        std::fs::create_dir_all(&dir).unwrap();
        let failures = vec![DeterministicFailure::EvaluationFailed {
            at: Site::FreeForward,
