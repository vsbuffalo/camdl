...
```
t     t_start  t_stop  S_child  E_child  I_child  R_child  S_adult  ...  flow_infection_child  flow_recovery_child  ...
0.0   0.0      0.0     499990   0        10       0        499995   ...  0                     0                    ...
7.0   0.0      7.0     499821   45       102      22       499990   ...  169                   58                   ...
14.0  7.0      14.0    499340   123      298      229      499978   ...  481                   171                  ...
```

Columns: `t`, `t_start`, `t_stop`; one column per compartment (state at `t`);
one column per transition prefixed with `flow_` (the flow accumulated over
`[t_start, t_stop)`, the previous output time up to this one). The authoritative
layout is `docs/camdl-run-spec.md` §10.1.
