...
**What (1) — forcing file columns are quoted strings.** In a `forcing { }`
block the file selectors — `data`, `time_col`, `value_col`, `key_col` — must now
be **quoted strings**; a bare identifier is rejected. This carries the reader's
...
`value_col = "C"`; `key_col = village` → `key_col = "village"`;
`data = f.tsv` → `data = "f.tsv"`. Leave `table = mymatrix` and
`time_dim = week` bare.

_Diagnostic._ A bare file selector → **E410** (`value_col must be a quoted
column name — write value_col = "C"`). A quoted `table`/`time_dim` → **E412**
...
_Diagnostic._ A quoted `method` → **E411** (`method is now a bare enum — write
method = linear`); an unknown value → **E411** (`unknown interpolation method
