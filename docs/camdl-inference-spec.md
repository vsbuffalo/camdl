...
On `camdl fit run fit.toml --stage scout` (or any fit command), the tool checks:
...
# Auto-generated by camdl fit run --stage scout.
...
```bash
camdl fit run fit.toml --stage scout [--seed 1]
```
...
The compound scout-convergence gate (read by `camdl fit run --stage refine`)
passes iff:
...
model.camdl + data.tsv + fit.toml
    │
    ├── camdl fit run --stage scout    → <fit_dir>/real/fit_<seed>/scout/fit_state.toml
    ├── camdl fit run --stage refine   → <fit_dir>/real/fit_<seed>/refine/mle_params.toml
    └── camdl fit run --stage validate → <fit_dir>/real/fit_<seed>/validate/mle_params.toml
                                      │
                    ┌─────────────────┤
                    ▼                  ▼
...
└────────────┬─────────────┘
             │ camdl fit run --stage scout → refine → validate
             ▼
┌──────────────────────────┐
