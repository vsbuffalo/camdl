...
output is a params.toml. It feeds directly into `camdl simulate`, `camdl batch`
(whose `[design.*]` block does space-filling sensitivity). No format conversion.
One chain from raw data to decision.
...
| `fit_state.toml`        | TOML   | Each method | The next method         | Inter-method handoff         |
| `{method}_summary.json` | JSON   | Each method | Agent / status          | Machine-readable diagnostics |
...
| `parameter_traces.tsv`  | TSV    | Each method | Plotting                | Per-iteration param values   |
| `diagnostics.tsv`       | TSV    | Each method | Plotting / analysis     | Per-iteration ESS, loglik    |
...
# (gh#241); the fit's `[method]` declares its own `backend`.
...
4. **Boundary proximity warning.** If the MLE is within 1% of a fit bound, warn
   ...
   This is reported in `{method}_summary.json` and `camdl fit summary`:
...
On `camdl fit run fit.toml` (or any fit command), the tool checks:
...
et al. 2015 PNAS, King et al. 2008 Nature, Bretó He Ionides King 2009 AOAS), and
the epistemic-laundering motivation.
...
`fit_state.toml` is the inter-method handoff. A method reads the previous
method's fit_state.toml and produces its own. The user never needs to edit it
(but can, for debugging).

### 4.2 Schema

```toml
# results/fits/<stem>-<h8>/if2-<h8>/seed_1-<h8>/fit_state.toml
# Auto-generated by camdl fit run.
...
#   CLI flag > [method.gate] > GateConfig::default()
...
A fit.toml carries one `[method]`; a chain of fits is a chain of files, each
naming the previous by handle:

```
scout.toml                                   [method] algorithm = "if2"
  │
  ├─ camdl fit run scout.toml --label scout
  │    reads: scout.toml (config + rw_sd)
  │    writes: results/fits/scout-<h8>/if2-<h8>/seed_<N>-<h8>/fit_state.toml
  │
refine.toml                                  [method] starts = { from_mle = "@scout" }
  │
  └─ camdl fit run refine.toml
       reads: refine.toml (config) + @scout's fit_state.toml (start_values)
       writes: results/fits/refine-<h8>/if2-<h8>/seed_<N>-<h8>/{fit_state.toml, mle_params.toml}
```

When the method declares `starts = { from_mle = "<handle>" }`:

- Starting values come from the source leaf's `fit_state.toml [start_values]`
  (every chain at that point; R̂ is not assessed, Â is annotated)
- rw_sd comes from this file's `[estimate]` rw_sd values, else auto-scaled
- Explicit `--rw-sd` CLI flags override everything

When no sourced rule is declared (`from_prior`, `uniform_unconstrained`, …):

- Starting values are drawn per chain under the rule; `single` uses model
  defaults + fit.toml `start` fields
...
### 6.1 Scout (an exploratory IF2 file)

```bash
camdl fit run scout.toml [--seed 1]
```
...
<fit_dir>/real/fit_<seed>/scout/
  ...
  fit_state.toml                    ← inter-method handoff
...
`<method>_summary.json` file written for IF2; that was a v1 artefact superseded
...
The compound IF2 convergence gate (read by a downstream file's
`starts = { from_mle = "@scout" }` resolution, and rendered by `fit summary`)
passes iff:
...
Defaults: `a_thresh = 1.01`, `decibans_thresh = 30.0`. Override with
`--decibans-thresh X`.
...
### 6.2 Refine (a second IF2 file, started at the scout's estimate)

```bash
# In refine.toml: [method] starts = { from_mle = "@scout" }
camdl fit run refine.toml [--seed 1]
```

**Defaults:** 4 chains, 1000 particles, 50 iterations, cooling_fraction=0.95
over 50 iterations.

**Reads:** the scout leaf's `fit_state.toml` for start values (resolved from the
`starts` handle).

**Writes:** same shape as scout (above). Use `camdl fit summary --format json`
for the interpretation surface (§7.2.2).

### 6.3 Validate (a third file)

```bash
# In validate.toml: [method] starts = { from_mle = "@refine" }
camdl fit run validate.toml [--seed 1]
```
...
Walks the fit dir, checks which method leaves have completed, reports
...
— rather than the pre-Phase-1 lie "(no completed leaves found)".
...
```json
{
  "schema": {"version": 2, "camdl_version": "0.1.0+abc1234"},
  "fit_dir": "fit/he2010",
  "methods": [
    {
      "name": "if2",
...
`# camdl ...` comment, no `[provenance]` block, no metadata. A fit carries one
method leaf per seed; the last completed leaf in label order is the one emitted.
...
model.camdl + data.tsv + fit.toml
    │
    ├── camdl fit run scout.toml     → results/fits/scout-<h8>/if2-<h8>/seed_<N>-<h8>/fit_state.toml
    ├── camdl fit run refine.toml    → results/fits/refine-<h8>/if2-<h8>/seed_<N>-<h8>/mle_params.toml
    └── camdl fit run validate.toml  → results/fits/validate-<h8>/if2-<h8>/seed_<N>-<h8>/mle_params.toml
                                      │
                    ┌─────────────────┤
                    ▼                  ▼
            design.toml          voi.toml
            (params = [mle])     (experiment = ...)
                    │                  │
                    ▼                  ▼
            camdl batch          (planned; not yet
            <design.toml>         implemented — today
                    │             camdl batch [design.*]
                    ▼             does space-filling
            sobol_indices.tsv     sensitivity)
                    │                  │
                    │                  ▼
                    │              evsi.tsv
                    │                  │
...
$ camdl fit run fit.toml --seed 1
  ...
  if2 already complete for these inputs (input_hash: 7f2c1d3a).
  Use --force to re-run.
```
...
mle_params.toml: ⚠ MODIFIED (content hash mismatch)
  ...
  To restore: camdl fit run validate.toml
              ([method] starts = { from_mle = "@refine" })
```
...
└────────────┬─────────────┘
             │ camdl fit run scout.toml → refine.toml → validate.toml
             ▼
┌──────────────────────────┐
