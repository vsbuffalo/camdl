...
| low          | low           | the reference's own recorded flows admit few predecessors, so the weight piles onto one ancestor before any screening; the filter weight is a factor too, but for integer compartments the density term dominates because it is frequently exactly `-inf`; the screen is not the lever and neither is reweighting — blocking, which stops holding those flows fixed, is |
