...
weighted mean over ten equal time bins, and its last bin is high in every run —
the segment of the path after the final observation is resampled freely whatever
the sampler's health. So a run whose early path is completely frozen still
averages a healthy-looking third. Measured on an 11-compartment stochastic Ebola
model, 103 daily observation times, post-burn-in means across 16 chains:
...
first — near 0 when renewal is uniform in time, near 1 when the genealogy has
coalesced. It also reports the **ancestor-sampling acceptance rate** beside
them, because a near-zero rate says the ancestor splice is contributing nothing
to renewal and the profile will not improve on its own. The per-sweep columns
are in each chain's `trace.tsv` as `trajectory_renewal` and
`renewal_b0 … renewal_b9`; bin `b` is the fraction of the series from `b/10` to
`(b+1)/10`, a fixed tenth, so profiles compare across models and particle
counts.
