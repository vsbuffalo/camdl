...
```
16 chains configured, 8 RAN, 6 sweeps, --parallel 14:
                                       15.15 s wall, 156.4 s user, 6.9 GB RSS
  → 8-chain fit at 8000 sweeps  ≈ 5.6 h wall, ≈ 46 CPU-h     (M4 Max, 14 cores)
  → if all 16 survived          ≈ 9-10 h wall, ≈ 88 CPU-h
```

**Read that first line carefully: `8 skipped via BadInit`.** Half the chains
were refused at initialisation at seed 7, so every wall figure below is an
8-chain figure. An earlier draft of this note paired the 8-chain wall (5.6 h)
with a 16-chain CPU total (87 CPU-h) — inconsistent by 1.53×, which is how the
refusals were found. That refusal rate is a **finding in its own right**: the
config's own header puts this family's baseline at 12.5%, and 16 chains was
chosen precisely because "8 chains has repeatedly left too few survivors to read
R-hat". At seed 7 it delivers exactly 8.
...
Cost per particle-substep ≈ **2.8 µs**. Scaling in particle count is linear (318
/ 281 / 266 / 259 µs per particle-sweep at 1200 / 2400 / 4800 / 9600), so there
is no per-sweep fixed cost worth attacking.
...
— and pushes four `Vec<Vec<_>>` histories per substep (`pgas.rs:2497–2500`).
That is ~6 M allocations per sweep. It is **invisible in the profile**
(`_platform_memmove` 0.24%, `RawVecInner::finish_grow` 0.13%).

The history _is_ the 6.9 GB RSS (9600 × 92 × ~512 B ≈ 452 MB/chain × 16 ≈ 7 GB,
matching the measurement), so gh#207 (leaner per-particle history) remains a
real **memory** lever — it is not a speed lever. Do not re-derive this.
...
`StatefulRng::binomial` (`rng.rs:219`) routes `n·min(p,1-p) < BINV_THRESHOLD` to
its own hand-owned BINV (correct; owns the gh#510 unbounded-loop and gh#525
large-`n` panic fixes — **do not touch it**) and everything else to
`rand_distr::Binomial`, which is BINV/BTPE per Kachitvichyanukul & Schmeiser
(1988), CACM 31(2):216–222.
...
3. **The mode-walk (step 5.1, lines 214–239) — the real cost.** The acceptance
   ...
   **one f64 division per step**. At `npq ≈ 190` the sd is ~13.8, so mean
   ...
   gives ~65; and the bench below MEASURES 49-63 ns at the total-exit regimes
   and 64-89 at the splits. An earlier draft asserted "the ~90–100 ns/draw the
...
`binomial` rather than BTPE. Expected gain here: BTPE ~32% of the fit → ~12–15%,
i.e. **~1.25–1.35× on the whole fit**. Inferred from the flop counts above, not
yet measured — the measurement is a `cargo bench` A/B on `rng.rs` before any
integration.
...
- **`rand_distr` version bump: no.** 0.4.3 is what is pinned
...
exists) — so 1.12× is available today with no code change.
...
| scope                                                            | factor     | fit time   |
| ---------------------------------------------------------------- | ---------- | ---------- |
| today, byte-identical, no code (`CAMDL_EVAL_FLAT` + build flags) | 1.07-1.12× | ~5.0-5.2 h |
| \+ BTRS binomial — **measured**                                  | **~1.25×** | ~4.5 h     |
| \+ non-cryptographic PRNG (inferred, NOT measured)               | ~1.3×      | ~4.3 h     |
| **hard ceiling** (binomial _and_ rate eval both free)            | **~3.2×**  | ~1.7 h     |

**BTRS is measured at 1.58× on the sampler → ~1.17× on the fit**
(`cargo bench -p sim --bench binomial_ab`: 1.54× at the total-exit regimes,
1.61× at the splits, 65 → 41 ns/draw blended). Flop counting predicted
1.25–1.35× _whole-fit_ and was wrong — by about the margin this repo's
[flat-evaluator note](2026-06-14-flat-bytecode-evaluator.md) records for exactly
this mistake (synthetic 2.5×, real corpus 1.27×). Recorded as wrong rather than
quietly dropped.
...
- **38.9% is a lower bound with ±0.8 pp of sampling noise** (14,331 samples at
  999 Hz). `rand_distr::Binomial::sample` is generic and monomorphises into
  `sim`'s codegen unit under a profile with no LTO, so it partially inlines —
  proven by `UniformFloat::<f64>::new` appearing as its own 0.59% leaf. Write
  "≈39%", not 38.9%.
...
4. **`sim`'s existing features scope _dependencies_, not numerics**
   ...
   feature would be a new and worse category on top of six existing _runtime_
   toggles (`CAMDL_EVAL_FLAT`, `CAMDL_EVAL_UNRESOLVED`,
   `CAMDL_NO_BINDING_CACHE`, `CAMDL_NO_LICM`, `CAMDL_NO_CONSTANT_FOLD`, ). A
   sixth, `CAMDL_SERIAL`, is a **ghost**: it appears only in prose — in
   [`2026-06-14-pgas-trajectory-io-bottleneck.md`](2026-06-14-pgas-trajectory-io-bottleneck.md)
...
- **During development:** a _temporary_ env-gated A/B (the pattern
  `CAMDL_EVAL_FLAT` and `CAMDL_NO_BINDING_CACHE` already established), used to
  prove distributional equivalence and to bench. **Deleted in the commit that
  makes BTRS the only sampler.** Env, not Cargo: it must be togglable inside a
  test harness without a rebuild.
...
- **Permanent, not temporary:** the equivalence test. BTPE and BTRS both sample
  ...
  this model actually visits (`np ≈ 87–192` on `n` from 10² to 6.3e6), plus the
  gh#510/gh#525 pathological inputs. This is the part that makes the change
  safe, and it outlives the toggle.
...
2. `lto = "fat"` + `codegen-units = 1` in `[profile.release]`;
   `target-cpu=native` only behind a green full `make test` — it is the one item
   here that could perturb FP.
3. `cargo bench -p sim` A/B for BTRS vs `rand_distr` BTPE at this model's
   `(n, p)` regimes, _before_ integrating.
4. Then BTRS, atomically, with the permanent distributional-equivalence suite.
5. Re-profile. Only then decide on the PRNG.
6. Independent of all of it: measure `trajectory_renewal` vs `N` on a _healthy_
...
**`CAMDL_EVAL_FLAT` does not appear to enter the run identity.** Two runs under
different evaluators wrote the same CAS leaf in the A/B above. Harmless while
the two are byte-identical, but it means an unrecorded input decides the
contents of a stored artifact — and it is the same provenance hole that argument
1 above rejects a Cargo feature for. Not filed.
