...
| Status     | Root cause confirmed by external review; fix not yet landed                                   |
...
recorded anywhere. It needs the same external scrutiny the sign got, and it
should be settled _before_ the sign fix lands — a corrected ratio derived for a
target the rungs do not have is still not right.
