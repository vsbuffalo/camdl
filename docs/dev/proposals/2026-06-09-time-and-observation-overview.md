...
| Capability gate — `required_capabilities()` vs each backend's `capabilities()`, plus the `(algorithm × backend)` registry       | **exists** — but note the deliberate sim-vs-fit fork: chain-binomial inference withholds `REAL_COMPARTMENTS` (gh#191) via a _second_, hardcoded table in `fit/methods.rs::check_model_capabilities`, distinct from `Simulate::capabilities()` which grants it. The two tables are an open known gap (`capabilities-system.md`, RFC `2026-06-08-capability-gate-consolidation.md` unimplemented), and the withholding is the _only_ guard against a silent real-compartment mis-fit — load-bearing, not "healthy" |
...
**`RunWindows` owns _orderings_, not coverage.** It enforces the window
orderings and "every window ⊆ D"; it structurally cannot check that every
forcing's covariate domain Eⱼ covers the run (overlap config 6), because Eⱼ
lives in the model data, not in this type. That coverage check is a _sibling_
responsibility at the forcing layer (interval-model §7.1 pt 5 / §9). "One
authority" means one place for the window axis — not one place for every
time-related check; the next reader should expect the coverage check at the
forcing seam, where forcing v3 (gh#119) owns it.
...
— PGAS persists latent trajectories, IF2/PMMH persist parameters only. Because
the variant is chosen by _what the fit actually persisted_, the "capability
absent" case is explicit: a bootstrap-PF fit that did **not** save its paths
(paths are optional output) degrades to `ParameterOnly` (re-filter from
`t_start`, then project) rather than failing — absent paths fall through to
re-filter, they don't error. It is the same idea as the future
`DifferentiableObjective` trait (value + gradient) that would let ODE→NUTS slot
in: **key on the capability, not the backend name.**
