...
**`ir/golden/` is not part of that set and must not be regenerated.** Its 17
files are a deliberately frozen v0.3 corpus whose whole purpose is to prove that
IR written against an older schema still deserializes
(`rust/tests/golden_deser.rs` says so in as many words, and no Makefile target
writes to the directory). Regenerating them would delete the cross-version
regression they exist to provide. A new field carrying serde's `default` leaves
them deserializing correctly, which is the property under test.
