...
- **Status:** Accepted — spec, implement against this. Steps 0–2 landed: the
  ...
  surface and the loader, and the reset/score split in full — a declared period
...
54 files under `ocaml/golden/`, regenerated by `make update-golden`.
...
| ----------------------------------- | ----: | -------------------------- |
| `ir/golden/`                        |    17 | hand-authored              |
| `rust/crates/sim/tests/fixtures/`   |    18 | mixed — some have `.camdl` |
| `tests/external/ode_oracle/models/` |     3 | has `.camdl`               |
| `tests/fixtures/ode_flow/`          |     1 | hand-authored              |

Because `covers` is omitted when undeclared, the edit is the `ir_version` and
`validated_by` strings alone — a textual bump, which is also safer than
recompiling, since several of these were compiled with particular `--set` flags
that a naive regeneration would drop.

There is a trap here worth naming, because it points the wrong way. These files
carry TWO version fields: `ir_version` is the schema version that tracks
`ir/VERSION` (currently kept up to date), while `model.version` is a separate
model-format field reading `0.3`. A comment in `rust/tests/golden_deser.rs`
describing the corpus as "frozen v0.3" refers to the latter. Read as the schema
version it suggests a deliberately-old-schema fixture set that must not be
touched — the opposite of the truth.

The Rust side is wider than the OCaml side: 62 `ObservationModel` literals
across 32 files must each name the new field, and `ObservationModel`'s
`ContentAddressed` impl exhaustively destructures the struct, so the field has
to be hashed or explicitly bound `_` with a reason before it compiles. It is
hashed: the declaration changes what the likelihood scores.
