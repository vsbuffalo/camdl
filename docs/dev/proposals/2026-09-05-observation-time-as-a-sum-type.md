...
| `period = from_spacing`                | `(previous row, this row]` — today's behaviour |
...
`from_spacing` is today's behaviour, spelled. It exists so a model migrates in
one line with **no change in results** — you write down what you are already
getting, and can then see whether it is what you meant. It warns on every use
and is not a supported long-term form.
...
one. A rule that guesses is silently wrong for someone. The error names the five
forms above.
...
| `Intervals`, legacy  | one time column + `period = from_spacing`              | any existing model, until migrated                      |
...
hash only when non-empty — `from_spacing` and a declared period must hash
differently, which means the field re-keys every model and orphans stored runs.
`--resume` breaks across the bump, since the fit config hash lives in
`resume_state.bin`.
...
1. **Red first.** A daily incidence stream with a one-day hole is accepted today
   and silently scores a two-day window. After: refused, naming both times.
...
1. **`Period` and `StreamTimes` in the IR and runtime**, with `from_spacing`
   lowering to today's behaviour. The bump and the goldens land here. No user
   visible change yet.
2. **The DSL surface** — `window_start`/`window_stop` column kinds and the
   `period` declaration, with the required-declaration rule and its diagnostics.
3. **Output columns**, closing the round trip; unblocks gh#830 and gh#831.
4. **`compare`'s window gate**, and the `fit predict` grid.
