...
| E114      | parse          | malformed `sum(...)` argument list — binders come first, the body last                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| E260–E262 | semantic       | observation, balance, simulation-block validation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| E263      | semantic       | a dimension name does not resolve. Three faces, distinguished by the span: a **table declaration** (`tables { C : age × aeg = … }` — gh#490, checked at the declaration even when nothing indexes the table, which also closes the zero-knot forcing of gh#491), an **index** position (`C[a, child]`, or a level that is not in a declared axis), and a **reduction binder** (`sum(b in aeg, …)` — gh#488, where an undeclared axis used to resolve to an empty domain and fold the whole term to `0.0` with no diagnostic). The "not a declared dimension" wording is emitted once per distinct name, at the earliest site that sees it |
| E264–E276 | semantic       | observation, balance, simulation-block validation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| E283      | semantic       | a reduction binder shadows a name it must not. Two faces: an **enclosing index/bound variable**, which first-match-wins resolution would silently rebind (emitted by `check_no_shadowing`); and a **declared global** — parameter, compartment, `let`, forcing, or table — where the binder substitutes each level into the global's place and the level name then surfaced as `E100: undeclared name '<level>'` (gh#495, emitted by `resolve_expr`'s ESum arm so it replaces those E100s rather than adding to them)                                                                                                                     |
...
| E287      | semantic       | partial dimension omission in a rate read: a compartment stratified over 2+ dimensions is indexed with some but not all dimensions (`E[a]` when `E` has `[age, latent_stage]`) — a partial index has no defined cell; index all dims or marginalize explicitly (bare name still sums over all dims). The hint offers two forms and both must COMPILE (gh#493): the index-all form echoes the user's own tokens and fills each dropped axis from the binder in scope (`<dim>` when there is none), and the marginalize form binds EVERY dropped axis in one flat `sum` with names that collide with nothing in scope                       |
