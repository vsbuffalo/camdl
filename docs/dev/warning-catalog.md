...
| E230–E242 | semantic       | observation, balance, simulation-block validation                                                                                                                                                                                                                                                                                 |
| E243      | semantic       | staged-residence `via <law>(...)` names an unsupported law; `erlang(stages, mean ⊻ rate)` lowers — `hyper_erlang` and other laws are not yet supported (staged-residence proposal §4)                                                                                                                                              |
| E244      | semantic       | `via erlang(...)` `stages` is missing or is not a positive-integer literal; `stages` sets how many sub-stage compartments exist (model structure), so it must be a fixed positive integer (proposal §3)                                                                                                                            |
| E245      | semantic       | `via erlang(...)` does not specify exactly one of `mean` / `rate`; `rate` is 1/`mean`, give one (staged-residence proposal §3)                                                                                                                                                                                                     |
...
| E248      | semantic       | `via` on an already-stratified compartment is not yet supported (deferred sub-phase); stage an unstratified compartment, or express the per-stratum staging with manual sub-stage compartments (proposal §7)                                                                                                                       |
...
| E250–E276 | semantic       | observation, balance, simulation-block validation                                                                                                                                                                                                                                                                                 |
