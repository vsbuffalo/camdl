...
| E263      | semantic       | a dimension name does not resolve. Two faces, distinguished by the span: an **index** position (`C[a, child]` on a table declared over an undeclared axis, or a level that is not in a declared axis), and a **reduction binder** (`sum(b in aeg, …)` — gh#488, where an undeclared axis used to resolve to an empty domain and fold the whole term to `0.0` with no diagnostic)                                                       |
