...
| E224      | semantic       | an inflow into a `via hyper_erlang(...)`-staged compartment also produces other compartments (`--> src + X`); a multi-destination inflow cannot be split across the mixture's entry branches (would double the drain / decouple the sibling) — use single-destination inflows (staged-residence proposal §4)                      |
