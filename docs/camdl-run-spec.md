...
| `perturb_only_at_t0` | bool                                 | `false`                       | Perturb at t=0 only, not at every observation — the IF2 schedule for an initial-state parameter. Read by `if2` stages and ignored by the rest; a config-load error only when the fit declares no `if2` stage at all. Required by `ic_free`. |
...
**IC-free support.** `ic_free = true` is honored only by `if2`, `pfilter`, and
plain `pmmh` (`methods.rs:581`). `pgas`, correlated `pmmh`, and the ODE
